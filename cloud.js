...
let CLOUD_CENTER = (window.CLOUD_CENTER_ID || "principal");
/* Permet de changer le "centre" utilisé pour toutes les requêtes cloud (données, photos,
   licence). Utilisé par app.js pour isoler chaque client par son propre code de licence,
   quand plusieurs centres partagent le même site/Worker. */
function cloudSetCenter(id){
  if(id) CLOUD_CENTER = String(id);
}
function cloudGetCenter(){ return CLOUD_CENTER; }
