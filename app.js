...
const PHARMA_LOGO = "onpb-logo-transparent.png";
/* Filières santé (Auxiliaire de Pharmacie, Délégué Médical) : diplôme avec logo de l'Ordre
   National des Pharmaciens du Bénin (au lieu du logo du ministère) et déclinaison verte. */
function isPharmaFiliere(filiereId){ return filiereId==="fil_pharma" || filiereId==="fil_delegue"; }
...
/* Règles de modification des dépenses/recettes :
   - Admin : peut toujours modifier/supprimer, quelle que soit la date.
   - Secrétariat : peut modifier/supprimer une entrée datée d'aujourd'hui, une seule fois
     (verrouillée ensuite). Les entrées d'hier ou plus anciennes sont réservées à l'admin. */
function canEditFinanceEntry(entry, r){
  if(r==="admin") return true;
  if(!entry) return false;
  return entry.date===todayISO() && !entry.editedBySecretariat;
}
...
function studentsByFiliereHTML(sp, filiereId){
  ...
  <div class="card">
    <div class="row"><div class="section-title" style="margin:0;">${isUnassigned? "🕓 Élèves non affectés" : "🎓 "+esc(f?f.nom:"Filière")} — élèves inscrits (${list.length})</div>
      ${list.length? `<button class="btn btn-ghost btn-sm no-print" data-action="download-students-list-pdf" data-id="${esc(filiereId)}">⬇️ Liste en PDF</button>` : ""}
    </div>
  ...
  return topBarHTML(spaceTitle) + `<div class="wrap" style="padding:16px 16px 60px;max-width:760px;">
    ...
    <div class="card">
      ...
      ${hist.length? `<div class="table-wrap" style="margin-top:10px;"><table><thead><tr><th>Date</th><th>Montant</th><th>Mode</th><th>Note</th><th class="no-print"></th></tr></thead><tbody>
        ${hist.map(p=>`<tr><td>${formatDate(p.date)}</td><td style="font-weight:700;">${formatFCFA(p.montant)}</td><td>${esc(p.mode||"-")}</td><td>${esc(p.note||"-")}</td><td class="no-print" style="white-space:nowrap;"><button class="btn btn-ghost btn-sm" data-action="print-receipt" data-id="${p.id}">🖨️ Quittance</button>${role==="admin"? ` <button class="btn btn-ghost btn-sm" data-action="edit-payment" data-id="${p.id}">✏️</button> <button class="btn btn-danger btn-sm" data-action="delete-payment" data-id="${p.id}">🗑️</button>` : ""}</td></tr>`).join("")}
...
/* ===================== EXPORT PDF : listes (élèves par filière / enseignants) =====================
   Contrairement à la quittance/au diplôme (capture d'une page HTML), une liste peut être longue :
   on construit donc le PDF directement avec jsPDF (texte + lignes), avec pagination automatique
   quand une page est pleine. Disponible dans l'espace Admin ET Secrétariat. */
function downloadListPDF(opts, btn){
  if(!window.jspdf){
    alert("La génération de PDF n'a pas pu être chargée. Vérifiez votre connexion internet au moins une fois, puis réessayez (elle sera ensuite disponible hors-ligne).");
    return;
  }
  ...
  try{
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
    const pageH = 297, marginX = 14, marginBottom = 18;
    ...
    function drawTableHeaderRow(){
      ...
      let x=startX;
      opts.columns.forEach((c,i)=>{ pdf.text(String(c.header), x, y); x+=colWidths[i]; });
      y+=1.5;
      pdf.setDrawColor(180); pdf.line(startX, y, startX+totalW, y);
      y+=5.5;
      pdf.setFont("helvetica","normal"); pdf.setFontSize(9);
    }
    function drawPageHeader(){
      pdf.setFont("helvetica","bold"); pdf.setFontSize(13);
      pdf.text(db.settings.centerName||"Centre de formation", startX, 16);
      pdf.setFont("helvetica","normal"); pdf.setFontSize(9);
      pdf.text(db.settings.centerSubtitle||"", startX, 21);
      pdf.setFont("helvetica","bold"); pdf.setFontSize(12);
      pdf.text(opts.title, startX, 30);
      y = 30;
      if(opts.subtitle){ y=35; pdf.setFont("helvetica","normal"); pdf.setFontSize(9.5); pdf.text(opts.subtitle, startX, y); }
      y += 7;
      drawTableHeaderRow();
    }
    drawPageHeader();
    opts.rows.forEach(row=>{
      ...
      let x=startX;
      row.forEach((cell,i)=>{
        pdf.text(String(cell==null||cell===""?"-":cell), x, y, {maxWidth: colWidths[i]-2});
        x += colWidths[i];
      });
      y += 6;
    });
    pdf.setFontSize(8); pdf.setTextColor(140);
    pdf.text(`Généré le ${formatDate(todayISO())} — ${opts.rows.length} élément(s)`, startX, pageH-8);
...
function downloadStudentsListPDF(filiereId, btn){
  const isUnassigned = filiereId==="unassigned";
  const f = getFiliere(filiereId);
  const list = activeStudents().filter(s=> isUnassigned ? !s.filiereId : s.filiereId===filiereId)
    .slice().sort((a,b)=> (a.nom||"").localeCompare(b.nom||""));
  const rows = list.map(s=>{
    const bal = studentBalance(s);
    return [s.matricule||"-", `${s.nom||""} ${s.prenom||""}`.trim(), s.sexe==="F"?"F":"M", s.telephone||"-", isUnassigned? "Filière à définir" : (bal>0? formatFCFA(bal)+" restant" : "Soldé")];
  });
  downloadListPDF({
    title: "Liste des élèves",
    subtitle: (isUnassigned? "Élèves non affectés" : (f?f.nom:"Filière")) + (db.settings.anneeScolaire? ` — ${db.settings.anneeScolaire}` : ""),
    columns: [
      {header:"Matricule", width:26},
      {header:"Nom & prénom", width:62},
      {header:"Sexe", width:14},
      {header:"Téléphone", width:32},
      {header:"Solde", width:46}
    ],
    rows,
    filename: `Liste_eleves_${(isUnassigned?"non_affectes":(f?f.nom:"filiere")).replace(/\s+/g,"_")}.pdf`
  }, btn);
}
function downloadTeachersListPDF(btn){
  const list = db.teachers.slice().sort((a,b)=> (a.nom||"").localeCompare(b.nom||""));
  const rows = list.map(t=>[`${t.nom||""} ${t.prenom||""}`.trim(), t.matiere||"-", t.telephone||"-", formatFCFA(t.salaireMensuel), formatDate(t.dateEmbauche)]);
  downloadListPDF({
    title: "Liste des enseignants",
    subtitle: db.settings.centerName||"",
    columns: [
      {header:"Nom & prénom", width:50},
      {header:"Matière", width:38},
      {header:"Téléphone", width:30},
      {header:"Salaire mensuel", width:32},
      {header:"Date d'embauche", width:28}
    ],
    rows,
...
function diplomeSealSVG(centerLabel, green){
  ...
  const centerFill = green ? "#0B4D2E" : "#0d1b3d";
  let starsRow="";
  ...
  return `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="60" cy="60" r="41" fill="${centerFill}" stroke="#c79a3f" stroke-width="1"/>
...
/* Coin haut-gauche : grand élément géométrique (bleu marine, ou vert pour les filières santé) avec courbe dorée élégante — vectoriel, net à l'impression */
function diplomeCornerTLSVG(green){
  const c1 = green ? "#0B4D2E" : "#0B1F4D";
  return `<svg viewBox="0 0 108 140" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
    ...
    <path d="M0,0 L54,0 C39,4 25,19 15,44 C7,65 3,88 0,118 Z" fill="${c1}"/>
...
/* Coin bas-droit : plusieurs bandes courbes superposées (bleu marine, ou vert), liserés dorés métalliques, effet de mouvement */
function diplomeCornerBRSVG(green){
  const c1 = green ? "#0B4D2E" : "#0B1F4D";
  const c2 = green ? "#146B3E" : "#12275a";
  const c3 = green ? "#083019" : "#0a1730";
  return `<svg viewBox="0 0 160 124" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
    ...
    <path d="M160,124 L160,26 C136,33 114,58 98,90 C88,110 76,120 62,124 Z" fill="${c1}"/>
    <path d="M160,26 C136,33 114,58 98,90 C88,110 76,120 62,124" fill="none" stroke="url(#brGold)" stroke-width="1.4"/>
    <path d="M160,124 L160,50 C140,56 121,76 108,102 C100,117 91,122 80,124 Z" fill="${c2}" opacity=".92"/>
    <path d="M160,50 C140,56 121,76 108,102 C100,117 91,122 80,124" fill="none" stroke="url(#brGold)" stroke-width="1.4"/>
    <path d="M160,124 L160,74 C144,79 128,94 118,114 C112,122 105,124 98,124 Z" fill="${c3}"/>
    <path d="M160,74 C144,79 128,94 118,114 C112,122 105,124 98,124" fill="none" stroke="url(#brGold)" stroke-width="1.4"/>
  </svg>`;
}
/* Grande vague en bas du diplôme (bleu marine, ou vert) (~17% de la hauteur) avec courbes dorées parallèles, fines et métalliques */
function diplomeWaveSVG(green){
  const c1 = green ? "#0B4D2E" : "#0B1F4D";
  return `<svg viewBox="0 0 990 100" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
    ...
    <path d="M0,34 C140,10 260,52 400,28 C540,4 640,50 780,26 C860,12 930,28 990,18 L990,100 L0,100 Z" fill="${c1}"/>
...
function diplomeHTML(s){
  const green = isPharmaFiliere(s.filiereId);
  const headerLogo = green ? PHARMA_LOGO : MINISTERE_LOGO;
  const headerLogoAlt = green ? "Ordre National des Pharmaciens du Bénin" : "Ministère des Enseignements Secondaire, Technique et de la Formation Professionnelle — République du Bénin";
  ...
  return `<div class="diplome-scale-wrap"><div class="diplome-canvas print-area${green?" diplome-canvas--green":""}">
    <div class="diplome-bgline"></div>
    ${wm}
    <div class="diplome-corner-tl">${diplomeCornerTLSVG(green)}</div>
    <div class="diplome-corner-br">${diplomeCornerBRSVG(green)}</div>
    <div class="diplome-wave">${diplomeWaveSVG(green)}</div>
    ...
    <div class="diplome-seal">${diplomeSealSVG(db.settings.centerSubtitle, green)}</div>
    ...
    <div class="diplome-header">
      <div class="diplome-header-left">
        <div class="diplome-header-left-logo"><img src="${headerLogo}" alt="${headerLogoAlt}"></div>
      </div>
      ...
      <div class="diplome-header-right-spacer"></div>
    </div>
    ...
    <ul class="diplome-vu-list">
      ${green ? `
      <li>Vu l'arrêté Ministériel No 105/MESTFP/DC/SGM/DIPIQ/DETFP/DPAF/SGSI/SA/045SGG22 portant création et ouverture du Centre Privé de Formation Appliquée et Professionnelle ${centerName} ;</li>
      <li>Vu le décret No 2019-500 du 13 Novembre 2019 portant organisation et fonctionnement de l'Ordre National des Pharmaciens du Bénin ;</li>
      <li>Vu la décision No 002/2023/ONPB du 29 Juin 2023 portant modalités de formation, recrutement, attributions et rémunérations des Auxiliaires en Pharmacie et de leur rapport avec les pharmaciens en République du Bénin ;</li>
      <li>Vu la décision du Conseil National de l'Ordre National des Pharmaciens du Bénin (CNOPB) en sa première session ordinaire du 27 et 28 février 2024 ;</li>
  ...
  <div class="card">
    ...
    ${recents.length? `<div class="table-wrap"><table><thead><tr><th>Date</th><th>Élève</th><th>Montant</th><th>Mode</th><th></th></tr></thead><tbody>
      ${recents.map(p=>{ const s=studentById(p.studentId); return `<tr>
        ...
        <td>${esc(p.mode||"-")}</td>
        <td style="white-space:nowrap;"><button class="btn btn-ghost btn-sm" data-action="print-receipt" data-id="${p.id}">🖨️ Quittance</button>${sp==="admin"? ` <button class="btn btn-ghost btn-sm" data-action="edit-payment" data-id="${p.id}">✏️</button> <button class="btn btn-danger btn-sm" data-action="delete-payment" data-id="${p.id}">🗑️</button>` : ""}</td>
      </tr>`; }).join("")}
    </tbody></table></div>` : `<div class="empty">Aucun paiement enregistré.</div>`}
  </div>`;
}
...
function depensesTabHTML(sp){
  ...
  <div class="card">
    ${list.length? `<div class="table-wrap"><table><thead><tr><th>Date</th><th>Motif</th><th>Montant</th><th>Mode</th><th></th></tr></thead><tbody>
      ${list.map(e=>{ const editable = canEditFinanceEntry(e, sp); return `<tr>
        <td>${formatDate(e.date)}</td><td>${esc(e.motif)}</td>
        <td style="font-weight:700;color:var(--red-600);">${formatFCFA(e.montant)}</td>
        <td>${esc(e.mode||"-")}</td>
        <td><span style="display:flex;gap:6px;align-items:center;">
          <button class="btn btn-ghost btn-sm" data-action="print-expense" data-id="${e.id}">🖨️</button>
          ${editable? `<button class="btn btn-ghost btn-sm" data-action="edit-expense" data-id="${e.id}">✏️</button>
          <button class="btn btn-danger btn-sm" data-action="delete-expense" data-id="${e.id}">🗑️</button>` : `<span class="pill" style="font-size:10.5px;">🔒 Admin</span>`}
        </span></td>
      </tr>`; }).join("")}
...
function recettesTabHTML(sp){
  ...
  <div class="card">
    ${list.length? `<div class="table-wrap"><table><thead><tr><th>Date</th><th>Motif</th><th>Montant</th><th>Mode</th><th></th></tr></thead><tbody>
      ${list.map(r=>{ const editable = canEditFinanceEntry(r, sp); return `<tr>
        ...
        <td><span style="display:flex;gap:6px;align-items:center;">
          ...
          ${editable? `<button class="btn btn-ghost btn-sm" data-action="edit-revenue" data-id="${r.id}">✏️</button>
          <button class="btn btn-danger btn-sm" data-action="delete-revenue" data-id="${r.id}">🗑️</button>` : `<span class="pill" style="font-size:10.5px;">🔒 Admin</span>`}
        </span></td>
      </tr>`; }).join("")}
...
const RAPPORTS_LIST = [
  ...
  {id:"elevesFiliere", icon:"📋", title:"Liste des élèves par filière", desc:"Liste imprimable des élèves inscrits dans une filière donnée.", color:"var(--navy-900)"},
...
function rapportHTML(kind){
  ...
  else if(kind==="elevesFiliere"){
    csvBtn = "";
    const filieres = db.filieres;
    if(!rapportFiliereId && filieres.length) rapportFiliereId = filieres[0].id;
    const list = activeStudents().filter(s=> rapportFiliereId==="__unassigned__" ? !s.filiereId : s.filiereId===rapportFiliereId)
      .slice().sort((a,b)=>(a.nom||"").localeCompare(b.nom||""));
    body = `<div class="filter-bar no-print">
      <div class="field"><span>Filière</span><select onchange="rapportFiliereId=this.value;render();">
        ${filieres.map(f=>`<option value="${esc(f.id)}" ${rapportFiliereId===f.id?'selected':''}>${esc(f.nom)}</option>`).join("")}
        <option value="__unassigned__" ${rapportFiliereId==="__unassigned__"?'selected':''}>Non affectés</option>
      </select></div>
    </div>
    <div class="card" style="margin-bottom:14px;"><div class="row"><span style="font-size:13px;color:var(--slate-600);">Effectif</span><span style="font-weight:800;">${list.length}</span></div></div>
    <div class="table-wrap"><table><thead><tr><th>Matricule</th><th>Nom & prénom</th><th>Sexe</th><th>Téléphone</th><th>Solde</th></tr></thead><tbody>
      ${list.length? list.map(s=>{ const bal=studentBalance(s); return `<tr><td>${esc(s.matricule||"-")}</td><td>${esc(s.nom)} ${esc(s.prenom)}</td><td>${s.sexe==="F"?"F":"M"}</td><td>${esc(s.telephone||"-")}</td><td>${bal>0? formatFCFA(bal)+" restant" : "Soldé"}</td></tr>`; }).join("") : `<tr><td colspan="5" style="text-align:center;color:var(--slate-400);">Aucun élève dans cette filière.</td></tr>`}
    </tbody></table></div>`;
  }
  ...
  return `<div class="wrap no-print" style="padding:14px 16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
    ...
    <div class="wrap print-area" style="max-width:900px;padding:20px 16px 60px;">
      <div class="doc-header">${logoBadgeHTML()}<div><div class="t1">${esc(db.settings.centerName)}</div><div class="t2">${esc(db.settings.centerSubtitle)} · ${esc(db.settings.site)} · ${esc(db.settings.phone)}</div></div></div>
...
function teachersTabHTML(sp){
  ...
  <div class="row" style="margin-bottom:14px;">
    ...
    <span style="display:flex;gap:8px;flex-wrap:wrap;">
      ${list.length? `<button class="btn btn-ghost" data-action="download-teachers-list-pdf">⬇️ Liste en PDF</button>` : ""}
  ...
  ${list.length? list.map(t=>{
    ...
    return `<div class="card">
      ...
      <div class="fab-bottom">
        ...
        ${sp==="admin"? `<button class="btn btn-ghost btn-sm" data-action="edit-teacher" data-id="${t.id}">✏️ Modifier</button> <button class="btn btn-danger btn-sm" data-action="delete-teacher" data-id="${t.id}">🗑️ Supprimer</button>` : ""}
      </div>
    </div>`;
...
function modalHTML(){
  ...
  else if(t==="editPayment") inner = editPaymentModalHTML();
  else if(t==="newTeacher") inner = teacherFormModalHTML();
  else if(t==="editTeacher") inner = teacherFormModalHTML(true);
  ...
  else if(t==="editExpense") inner = expenseFormModalHTML(true);
  else if(t==="newRevenue") inner = revenueFormModalHTML();
  else if(t==="editRevenue") inner = revenueFormModalHTML(true);
...
function studentFormModalHTML(isEdit){
  ...
  return `<div class="modal-title">${isEdit? "✏️ Modifier l'élève" : "🎓 Nouvelle inscription"}</div>
    ...
    <div class="fab-bottom">
      <button class="btn btn-ghost btn-full" data-action="close-modal">Annuler</button>
      <button class="btn btn-primary btn-full" data-action="${isEdit?'submit-edit-student':'submit-new-student'}" data-id="${isEdit && s ? s.id : ''}">💾 Enregistrer</button>
    </div>`;
}
...
/* Modification d'une quittance existante (réservé à l'Admin) : contrairement à l'encaissement,
   on ne change pas l'élève ni le motif, seulement montant/date/mode/note. */
function editPaymentModalHTML(){
  const p = paymentById(modalState.id);
  if(!p) return `<div class="modal-title">Quittance introuvable</div><div class="fab-bottom"><button class="btn btn-ghost btn-full" data-action="close-modal">Fermer</button></div>`;
  const s = studentById(p.studentId);
  return `<div class="modal-title">✏️ Modifier la quittance N° ${esc(p.receiptNo||p.id)}</div>
    <div class="field"><span>Élève</span><input value="${esc(s? s.nom+' '+s.prenom : 'Élève supprimé')}" disabled></div>
    <div class="field"><span>Montant versé (FCFA)</span><input id="pay-edit-montant" type="number" min="0" value="${Number(p.montant)||0}"></div>
    <div class="field"><span>Date</span><input id="pay-edit-date" type="date" value="${p.date||todayISO()}"></div>
    <div class="field"><span>Mode de paiement</span><select id="pay-edit-mode">
      ${["Espèces","Mobile Money","Virement","Chèque"].map(m=>`<option ${p.mode===m?'selected':''}>${m}</option>`).join("")}
    </select></div>
    <div class="field"><span>Note (optionnel)</span><input id="pay-edit-note" value="${esc(p.note||"")}"></div>
    <div class="fab-bottom">
      <button class="btn btn-ghost btn-full" data-action="close-modal">Annuler</button>
      <button class="btn btn-primary btn-full" data-action="submit-edit-payment" data-id="${p.id}">💾 Enregistrer</button>
    </div>`;
}
function submitEditPayment(id){
  if(role!=="admin"){ showToast("Seul l'administrateur peut modifier une quittance.", "err"); return; }
  const p = paymentById(id);
  if(!p){ showToast("Paiement introuvable.", "err"); return; }
  const montant = Number(document.getElementById("pay-edit-montant").value);
  if(!montant || montant<=0){ showToast("Montant invalide.", "err"); return; }
  p.montant = montant;
  p.date = document.getElementById("pay-edit-date").value || p.date;
  p.mode = document.getElementById("pay-edit-mode").value;
  p.note = document.getElementById("pay-edit-note").value.trim();
  saveDB(); modalState=null; showToast("Quittance mise à jour."); render();
}
...
function teacherFormModalHTML(isEdit){
  const t = isEdit ? teacherById(modalState.id) : null;
  return `<div class="modal-title">${isEdit? "✏️ Modifier l'enseignant" : "🧑‍🏫 Nouvel enseignant"}</div>
    <div class="grid2">
      <div class="field"><span>Nom</span><input id="tc-nom" value="${t?esc(t.nom):''}"></div>
      <div class="field"><span>Prénom</span><input id="tc-prenom" value="${t?esc(t.prenom):''}"></div>
    </div>
    <div class="field"><span>Matière / filière enseignée</span><input id="tc-matiere" value="${t?esc(t.matiere||''):''}"></div>
    <div class="field"><span>Téléphone</span><input id="tc-tel" value="${t?esc(t.telephone||''):''}"></div>
    <div class="grid2">
      <div class="field"><span>Salaire mensuel (FCFA)</span><input id="tc-salaire" type="number" min="0" value="${t?Number(t.salaireMensuel)||0:''}"></div>
      <div class="field"><span>Date d'embauche</span><input id="tc-embauche" type="date" value="${t?t.dateEmbauche:todayISO()}"></div>
    </div>
    <div class="fab-bottom">
      <button class="btn btn-ghost btn-full" data-action="close-modal">Annuler</button>
      <button class="btn btn-primary btn-full" data-action="${isEdit?'submit-edit-teacher':'submit-new-teacher'}" data-id="${t?t.id:''}">💾 Enregistrer</button>
    </div>`;
}
...
function expenseFormModalHTML(isEdit){
  const e = isEdit ? (db.expenses||[]).find(x=>x.id===modalState.id) : null;
  return `<div class="modal-title">${isEdit? "✏️ Modifier la dépense" : "💸 Nouvelle dépense"}</div>
    <div class="field"><span>Motif</span><input id="ex-motif" placeholder="Ex: Achat fournitures" value="${esc(e?e.motif:'')}"></div>
    <div class="grid2">
      <div class="field"><span>Montant (FCFA)</span><input id="ex-montant" type="number" min="0" value="${e?Number(e.montant)||0:''}"></div>
      <div class="field"><span>Date</span><input id="ex-date" type="date" value="${e?e.date:todayISO()}"></div>
    </div>
    <div class="field"><span>Mode</span><select id="ex-mode">
      ${["Espèces","Mobile Money","Virement","Chèque"].map(m=>`<option ${e&&e.mode===m?'selected':''}>${m}</option>`).join("")}
    </select></div>
    <div class="fab-bottom">
      <button class="btn btn-ghost btn-full" data-action="close-modal">Annuler</button>
      <button class="btn btn-primary btn-full" data-action="${isEdit?'submit-edit-expense':'submit-expense'}" data-id="${e?e.id:''}">💾 Enregistrer</button>
    </div>`;
}

function revenueFormModalHTML(isEdit){
  const r = isEdit ? (db.revenues||[]).find(x=>x.id===modalState.id) : null;
  return `<div class="modal-title">${isEdit? "✏️ Modifier la recette" : "💰 Nouvelle recette"}</div>
    <div class="field"><span>Motif</span><input id="rv-motif" placeholder="Ex: Subvention, don..." value="${esc(r?r.motif:'')}"></div>
    <div class="grid2">
      <div class="field"><span>Montant (FCFA)</span><input id="rv-montant" type="number" min="0" value="${r?Number(r.montant)||0:''}"></div>
      <div class="field"><span>Date</span><input id="rv-date" type="date" value="${r?r.date:todayISO()}"></div>
    </div>
    <div class="field"><span>Mode</span><select id="rv-mode">
      ${["Espèces","Mobile Money","Virement","Chèque"].map(m=>`<option ${r&&r.mode===m?'selected':''}>${m}</option>`).join("")}
    </select></div>
    <div class="fab-bottom">
      <button class="btn btn-ghost btn-full" data-action="close-modal">Annuler</button>
      <button class="btn btn-primary btn-full" data-action="${isEdit?'submit-edit-revenue':'submit-revenue'}" data-id="${r?r.id:''}">💾 Enregistrer</button>
    </div>`;
}
...
function onAppClick(e){
  ...
  switch(action){
    ...
    case "submit-edit-student": submitEditStudent(id || (modalState && modalState.id)); break;
    ...
    case "edit-payment":
      if(role!=="admin"){ showToast("Seul l'administrateur peut modifier une quittance.", "err"); break; }
      modalState={type:"editPayment", id}; render(); break;
    case "submit-edit-payment": submitEditPayment(id); break;
    case "delete-payment": {
      if(role!=="admin"){ showToast("Seul l'administrateur peut annuler une quittance.", "err"); break; }
      const p = paymentById(id);
      const pLabel = p ? (formatFCFA(p.montant) + " (N\u00b0 " + esc(p.receiptNo||p.id) + ")") : "";
      modalState={type:"confirm", title:"Annuler la quittance", msg:"Cette action supprimera d\u00e9finitivement le paiement de "+pLabel+". Continuer ?", onYes:()=>{ db.payments = db.payments.filter(pay=>pay.id!==id); saveDB(); showToast("Quittance annul\u00e9e."); }};
      render(); break;
    }
    ...
    case "download-students-list-pdf": downloadStudentsListPDF(id, btn); break;
    case "download-teachers-list-pdf": downloadTeachersListPDF(btn); break;
    ...
    case "edit-teacher": {
      if(role!=="admin"){ showToast("Seul l'administrateur peut modifier un enseignant.","err"); break; }
      modalState={type:"editTeacher", id}; render(); break;
    }
    case "submit-edit-teacher": submitEditTeacher(id); break;
    ...
    case "edit-expense": {
      if(!canEditFinanceEntry(expenseById(id), role)){ showToast("Modification réservée à l'administrateur pour cette dépense.","err"); break; }
      modalState={type:"editExpense", id}; render(); break;
    }
    case "submit-edit-expense": submitEditExpense(id); break;
    case "print-expense": screen={type:"expenseReceipt", id}; render(); break;
    case "delete-expense": {
      if(!canEditFinanceEntry(expenseById(id), role)){ showToast("Suppression réservée à l'administrateur pour cette dépense.","err"); break; }
      modalState={type:"confirm", title:"Supprimer la dépense", msg:"Continuer ?", onYes:()=>{ db.expenses = (db.expenses||[]).filter(e=>e.id!==id); saveDB(); showToast("Dépense supprimée."); }}; render(); break;
    }

    case "new-revenue": modalState={type:"newRevenue"}; render(); break;
    case "submit-revenue": submitRevenue(); break;
    case "edit-revenue": {
      if(!canEditFinanceEntry(revenueById(id), role)){ showToast("Modification réservée à l'administrateur pour cette recette.","err"); break; }
      modalState={type:"editRevenue", id}; render(); break;
    }
    case "submit-edit-revenue": submitEditRevenue(id); break;
    case "print-revenue": screen={type:"revenueReceipt", id}; render(); break;
    case "delete-revenue": {
      if(!canEditFinanceEntry(revenueById(id), role)){ showToast("Suppression réservée à l'administrateur pour cette recette.","err"); break; }
      modalState={type:"confirm", title:"Supprimer la recette", msg:"Continuer ?", onYes:()=>{ db.revenues = (db.revenues||[]).filter(r=>r.id!==id); saveDB(); showToast("Recette supprimée."); }}; render(); break;
    }
...
function submitEditStudent(id){
  const s = studentById(id);
  if(!s){ showToast("Élève introuvable, la modification n'a pas pu être enregistrée.", "err"); return; }
  readPhotoFile(function(photoData){
...
function submitEditTeacher(id){
  if(role!=="admin"){ showToast("Seul l'administrateur peut modifier un enseignant.","err"); modalState=null; render(); return; }
  const t = teacherById(id);
  if(!t){ showToast("Enseignant introuvable.","err"); return; }
  const nom = document.getElementById("tc-nom").value.trim();
  const prenom = document.getElementById("tc-prenom").value.trim();
  if(!nom || !prenom){ showToast("Le nom et le prénom sont obligatoires.","err"); return; }
  t.nom = nom; t.prenom = prenom;
  t.matiere = document.getElementById("tc-matiere").value.trim();
  t.telephone = document.getElementById("tc-tel").value.trim();
  t.salaireMensuel = Number(document.getElementById("tc-salaire").value)||0;
  t.dateEmbauche = document.getElementById("tc-embauche").value || t.dateEmbauche;
  saveDB(); modalState=null; showToast("Enseignant mis à jour."); render();
}
...
function submitEditExpense(id){
  ...
  if(!e){ showToast("Dépense introuvable.","err"); return; }
  if(!canEditFinanceEntry(e, role)){ showToast("Modification réservée à l'administrateur pour cette dépense.","err"); modalState=null; render(); return; }
  const motif = document.getElementById("ex-motif").value.trim();
  const montant = Number(document.getElementById("ex-montant").value);
  if(!motif || !montant || montant<=0){ showToast("Motif et montant valides sont obligatoires.","err"); return; }
  e.motif = motif; e.montant = montant;
  e.date = document.getElementById("ex-date").value || e.date;
  e.mode = document.getElementById("ex-mode").value;
  if(role==="secretariat") e.editedBySecretariat = true;
  saveDB(); modalState=null; showToast("Dépense mise à jour."); render();
}
...
function submitEditRevenue(id){
  const r = (db.revenues||[]).find(x=>x.id===id);
  if(!r){ showToast("Recette introuvable.","err"); return; }
  if(!canEditFinanceEntry(r, role)){ showToast("Modification réservée à l'administrateur pour cette recette.","err"); modalState=null; render(); return; }
  const motif = document.getElementById("rv-motif").value.trim();
  const montant = Number(document.getElementById("rv-montant").value);
  if(!motif || !montant || montant<=0){ showToast("Motif et montant valides sont obligatoires.","err"); return; }
  r.motif = motif; r.montant = montant;
  r.date = document.getElementById("rv-date").value || r.date;
  r.mode = document.getElementById("rv-mode").value;
  if(role==="secretariat") r.editedBySecretariat = true;
  saveDB(); modalState=null; showToast("Recette mise à jour."); render();
}
